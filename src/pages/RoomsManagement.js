...
useEffect(() => {
  ...
  const handleAddRoom = async () => {
    try {
      await axios.post(`${process.env.REACT_APP_API}/rooms`, newRoom, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDeleteRoom = async (id) => {
    if (window.confirm("Bạn có chắc muốn xóa phòng này?")) {
      try {
        await axios.delete(`${process.env.REACT_APP_API}/rooms/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleUpdateRoom = async () => {
    try {
      await axios.put(`${process.env.REACT_APP_API}/rooms/${editRoom._id}`, editRoom, {
        headers: { Authorization: `Bearer ${token}` }
      });
